    ...
    async def rollback_update(
        ...
        *,
        ...
        if preserve_zone_attrs:
            zone = self.coordinator.get_attr(ATTR_DEVICETRACKER_ZONE)
            zone_name = self.coordinator.get_attr(ATTR_DEVICETRACKER_ZONE_NAME)
        await self.coordinator.restore_previous_attr(previous_attr)
        if preserve_zone_attrs:
            if self.coordinator.get_attr(ATTR_DEVICETRACKER_ZONE) != zone:
                self.coordinator.set_attr(ATTR_DEVICETRACKER_ZONE, zone)
            if self.coordinator.get_attr(ATTR_DEVICETRACKER_ZONE_NAME) != zone_name:
                self.coordinator.set_attr(ATTR_DEVICETRACKER_ZONE_NAME, zone_name)
        _LOGGER.debug(
